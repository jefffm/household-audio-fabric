...
unshare $ns_flags sh -eu -c '
  ...
  # RTSP and bounded AP2 dynamic TCP ports, both families and all trust failures.
  for port in 7000 6001 6010; do
    timeout 3 socat -u TCP4-LISTEN:$port,reuseaddr /dev/null & l=$!; sleep .1; echo ok | timeout 3 nsenter -t $p1 -n socat -u - TCP4:192.0.2.1:$port; wait $l
    timeout 3 socat -u TCP6-LISTEN:$port,reuseaddr /dev/null & l=$!; sleep .1; echo ok | timeout 3 nsenter -t $p1 -n socat -u - TCP6:[2001:db8:1::1]:$port; wait $l
    timeout 3 socat -u TCP4-LISTEN:$port,reuseaddr /dev/null & l=$!; sleep .1; if echo bad | timeout 3 nsenter -t $p1 -n socat -u -T1 - TCP4:192.0.2.1:$port,bind=198.51.100.2; then exit 79; fi; kill $l 2>/dev/null ||:
    timeout 3 socat -u TCP6-LISTEN:$port,reuseaddr /dev/null & l=$!; sleep .1; if echo bad | timeout 3 nsenter -t $p1 -n socat -u -T1 - TCP6:[2001:db8:1::1]:$port,bind=[2001:db8:2::2]; then exit 80; fi; kill $l 2>/dev/null ||:
    timeout 3 socat -u TCP4-LISTEN:$port,reuseaddr /dev/null & l=$!; sleep .1; if echo bad | timeout 3 nsenter -t $p2 -n socat -u -T1 - TCP4:192.0.2.3:$port; then exit 81; fi; kill $l 2>/dev/null ||:
    timeout 3 socat -u TCP6-LISTEN:$port,reuseaddr /dev/null & l=$!; sleep .1; if echo bad | timeout 3 nsenter -t $p2 -n socat -u -T1 - TCP6:[2001:db8:1::3]:$port; then exit 82; fi; kill $l 2>/dev/null ||:
  done
  # PTP and bounded AP2 dynamic UDP ports, both families and all trust failures.
  for port in 319 320 6001 6010; do
